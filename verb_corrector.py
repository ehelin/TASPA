...
# -*- coding: utf-8 -*-
"""
Spanish Verb Conjugation Corrector
...
class SpanishVerbCorrector:
    ...
    def is_stem_changing_verb(self, infinitive: str, present_yo: str) -> bool:
        """Check if verb shows stem changes"""
        stem = infinitive[:-2]
        yo_stem = present_yo[:-1] if present_yo.endswith('o') else present_yo[:-2]
        return stem != yo_stem

    def get_stem_change_pattern(self, infinitive: str, present_forms: Dict[str, str]) -> Optional[str]:
        """Identify stem change pattern (e->ie, o->ue, e->i, etc.)"""
        stem = infinitive[:-2]
        
        # Check yo form vs tu form for stem changes
        yo = present_forms.get('Yo', '')
        tu = present_forms.get('Tu', '')
        
        if not yo or not tu:
            return None
            
        # Remove endings to get stems
        yo_stem = yo[:-1] if yo.endswith('o') else yo[:-2]
        tu_stem = tu[:-2] if tu.endswith('as') or tu.endswith('es') else tu[:-3]
        
        # Common patterns
        if 'ie' in yo_stem and 'e' in stem:
            return 'e->ie'
        elif 'ue' in yo_stem and 'o' in stem:
            return 'o->ue'
        elif 'i' in yo_stem and 'e' in stem and infinitive.endswith('ir'):
            return 'e->i'
    ...
    def generate_regular_subjunctive(self, verb_data: Dict) -> Dict:
        ...
        imp_endings = ['a', 'as', 'a', 'amos', 'ais', 'an']
    ...
    def process_verb_file(self, filepath: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            original_data = json.loads(content)  # Keep original for comparison
            ...
            # Even if no corrections, ensure proper formatting
            if json.dumps(original_data, ensure_ascii=False) != json.dumps(verb_data, ensure_ascii=False, indent=2):
                with open(filepath, 'w', encoding='utf-8') as f:
                    json.dump(verb_data, f, ensure_ascii=False, indent=2)
                corrections_made.append("Applied proper JSON formatting")
                self.common_errors['formatting_issues'] += 1
                return True, corrections_made
    ...
    def process_all_files(self):
        ...
        print(f"Processing {len(json_files)} verb files...")
